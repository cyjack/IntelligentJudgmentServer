...
class Vocab(object):
    ...
    def load_pretrained_embeddings(self, embedding_path):
        ...
        for line in f:
            ...
            if token not in self.token2id:
                continue
            VL = s[1:]
            vocter = [float(v) for v in VL]
            trained_embeddings[token] = vocter
            if self.embed_dim is None:
                self.embed_dim = len(s) - 1
    ...
    def load_CBOW_pretrained_embeddings(self, embedding_path):
        """
        loads the pretrained embeddings from embedding_path,
        tokens not in pretrained embeddings will be filtered
        Args:
        ...
        models = Word2Vec.load(embedding_path)
        vocab = models.wv.vocab
        word_vector = {}
        for word in vocab:
            word_vector[word] = models[word]
        for word in word_vector:
            token = word
            if token not in self.token2id:
                continue
            vocter = word_vector[word]
            trained_embeddings[token] = vocter
            if self.embed_dim is None:
                self.embed_dim = len(vocter)
